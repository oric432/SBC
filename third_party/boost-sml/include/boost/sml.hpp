...
struct join<
    ...
    type_list<T7s...>,
    type_list<T8s...>,
    Us...> : join<type_list<T1s..., T2s..., T3s..., T4s..., T5s..., T6s..., T7s..., T8s...>, Us...> {};
template <
    class... T01s,
    class... T02s,
    class... T03s,
...
struct add_unique {
    using type = conditional_t<
        is_base_of<type_wrapper<T>, Accumulator>::value,
        Accumulator,
        inherit<Accumulator, type_wrapper<T>>>;
};
template <class Accumulator, class T>
...
struct is_unique<type_wrapper<Rs...>, T, Ts...> : conditional_t<
                                                      is_base_of<type_wrapper<T>, inherit<type_wrapper<Rs>...>>::value,
                                                      false_type,
                                                      is_unique<type_wrapper<Rs..., T>, Ts...>> {};
template <class... Ts>
...
struct pool_type_impl<
    T&,
    aux::enable_if_t<aux::is_constructible<T>::value && aux::is_constructible<T, const T&>::value>> : pool_type_base {
...
struct zero_wrapper<R (TBase::*)(TArgs...), T> {
    ...
    constexpr explicit zero_wrapper(type ptr)
        : ptr{ptr} {}
...
struct zero_wrapper<R (*)(TArgs...), T> {
    using type = R (*)(TArgs...);
    explicit zero_wrapper(type ptr)
        : ptr{ptr} {}
...
using get_history_states = aux::join_t<
    typename aux::conditional<!Ts::history && Ts::initial, aux::type_list<typename Ts::src_state>, aux::type_list<>>::
        type...>;
template <class>
...
struct transitions<T, Ts...> {
    ...
    execute(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs, typename SM::state_t& current_state) {
...
struct transitions_sub<sm<Tsm>, T, Ts...> {
    ...
    constexpr static bool
    execute(const anonymous& event, SM& sm, TDeps& deps, TSubs& subs, typename SM::state_t& current_state) {
    ...
    template <class TEvent, class SM, class TDeps, class TSubs>
    constexpr static bool
    execute_impl(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs, typename SM::state_t& current_state) {
    ...
    constexpr static bool execute_impl(
        const back::on_entry<_, TEvent>& event,
        SM& sm,
        TDeps& deps,
        TSubs& subs,
    ...
    template <class _, class TEvent, class SM, class TDeps, class TSubs>
    constexpr static bool execute_impl(
        const back::on_exit<_, TEvent>& event,
        SM& sm,
        TDeps& deps,
        TSubs& subs,
        typename SM::state_t& current_state) {
...
struct conditional_mapping<false, aux::type_wrapper<T1, aux::inherit<Rs...>>, T2, Ts...> {
    using type = unique_mappings_impl<
        aux::type_wrapper<aux::inherit<T1, aux::type_wrapper<typename T2::element_type>>, aux::inherit<T2, Rs...>>,
        Ts...>;
};
template <class T1, class T2, class... Rs, class... Ts>
struct unique_mappings_impl<aux::type_wrapper<T1, aux::inherit<Rs...>>, T2, Ts...>
    : conditional_mapping<
          aux::is_base_of<aux::type_wrapper<typename T2::element_type>, T1>::value,
          aux::type_wrapper<T1, aux::inherit<Rs...>>,
          T2,
          Ts...>::type {};
...
struct mappings;
template <class... Ts>
struct mappings<aux::pool<Ts...>> : unique_mappings_t<event_mappings<
                                        typename Ts::event,
...
struct get_state_mapping {
    ...
    struct combined_mapping {
        ...
        constexpr static bool
        execute(const TEvent& event, TSm& sm, TDeps& deps, TSubs& subs, typename TSm::state_t& current_state) {
...
template <class SM, class TLogger, class TDeps, class TAction, class TEvent>
constexpr void log_action(
    const aux::type_wrapper<TLogger>&,
    TDeps& deps,
...
constexpr void
log_guard(const aux::type_wrapper<TLogger>&, TDeps& deps, const TGuard& guard, const TEvent& event, bool result) {
    return static_cast<aux::pool_type<TLogger&>&>(deps).value.template log_guard<SM>(guard, event, result);
}
template <class SM, class TLogger, class TDeps, class TGuard, class TEvent>
constexpr void log_guard(
    const aux::type_wrapper<TLogger>&,
    TDeps& deps,
...
struct sm_impl
    ...
    constexpr bool process_event(const TEvent& event, TDeps& deps, TSubs& subs) {
        ...
        do {
            ...
            subs,
            queued_handled,
            aux::type_wrapper<process_queue_t<TEvent>>{},
            events_t{}));
    ...
    template <
        ...
        __BOOST_SML_REQUIRES(aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value)>
    constexpr bool
    process_internal_generic_event(const TEvent& event, TDeps& deps, TSubs& subs, state_t& current_state) {
        policies::log_process_event<sm_t>(aux::type_wrapper<logger_t>{}, deps, event);
    #if BOOST_SML_DISABLE_EXCEPTIONS
        return process_event_impl<get_event_mapping_t<get_generic_t<TEvent>, mappings>>(
            event,
    ...
    constexpr bool process_event_impl(
        ...
        (void)aux::swallow{
            0,
            (handled |=
             dispatch_t::template dispatch<0, TMappings>(*this, current_state_[Ns], event, deps, subs, states),
             0)...};
    #endif
        return handled;
    }
    ...
    template <class TMappings, class TEvent, class TDeps, class TSubs>
    constexpr bool
    process_event_except_impl(const TEvent& event, TDeps& deps, TSubs& subs, state_t& current_state, aux::false_type) {
        return process_event_impl<TMappings>(event, deps, subs, states_t{}, current_state);
    }
    template <class TMappings, class TEvent, class TDeps, class TSubs>
    bool process_event_except_impl(
        const TEvent& event,
        TDeps& deps,
        TSubs& subs,
        state_t& current_state,
    ...
    constexpr bool process_defer_events(
        ...
        TSubs&,
        const bool,
        const aux::type_wrapper<no_policy>&,
    ...
    constexpr void
    visit_current_states(const TVisitor& visitor, const aux::type_list<TStates...>&, aux::index_sequence<0>) const {
    ...
    constexpr void
    visit_current_states(const TVisitor& visitor, const aux::type_list<TStates...>&, aux::index_sequence<Ns...>) const {
        using dispatch_table_t = void (*)(const TVisitor&);
        const dispatch_table_t dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(sizeof...(TStates))] = {
            &sm_impl::visit_state<TVisitor, TStates>...};
    #if defined(__cpp_fold_expressions)
    ...
    constexpr bool is_terminated_impl(aux::index_sequence<Ns...>) const {
        ...
        (void)aux::swallow{
            0,
            (result &= current_state_[Ns] == aux::get_id<state_t, terminate_state>((states_ids_t*)0))...};
        return result;
    #endif
    }
...
get_arg(const aux::type_wrapper<const TEvent&>&, const back::unexpected_event<T, TEvent>& event, Tsm&, TDeps&) {
    return event.event_;
}
template <class T, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto)
get_arg(const aux::type_wrapper<const TEvent&>&, const back::on_entry<T, TEvent>& event, Tsm&, TDeps&) {
    return event.event_;
}
template <class T, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto)
get_arg(const aux::type_wrapper<const TEvent&>&, const back::on_exit<T, TEvent>& event, Tsm&, TDeps&) {
    return event.event_;
}
template <class T, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto)
get_arg(const aux::type_wrapper<const TEvent&>&, const back::exception<T, TEvent>& event, Tsm&, TDeps&) {
...
struct call<TEvent, aux::type_list<Ts...>, TLogger> {
    ...
    execute_impl(const aux::type_wrapper<bool>&, T object, const TEvent& event, SM& sm, TDeps& deps) {
    ...
    template <class Tsm, class T, class SM, class TDeps>
    constexpr static auto
    execute_impl(const aux::type_wrapper<void>&, T object, const TEvent& event, SM& sm, TDeps& deps) {
        back::policies::log_action<Tsm>(aux::type_wrapper<TLogger>{}, deps, object, event);
...
constexpr void update_current_state(
    ...
    back::policies::log_state_change<typename SM::sm_t>(
        aux::type_wrapper<typename SM::logger_t>{},
        deps,
...
struct transition<state<S1>, state<S2>, front::event<E>, always, none> {
    ...
    template <class TEvent, class SM, class TDeps, class TSubs>
    constexpr bool
    execute(const TEvent&, SM& sm, TDeps& deps, TSubs& subs, typename SM::state_t& current_state, aux::false_type) {
        update_current_state(
            sm,
            deps,
            subs,
...
template <
    class... Ts,
    __BOOST_SML_REQUIRES(
        aux::is_same<aux::bool_list<aux::always<Ts>::value...>, aux::bool_list<concepts::transitional<Ts>::value...>>::
            value)>
