...
TEST_CASE("OptionsSm initial state is idle", "[options_sm]") {
    ...
    OptionsSm machine{actions};

    REQUIRE(machine.is_in(OptionsIdle{}));
    REQUIRE_FALSE(machine.is_in(OptionsResponding{}));
    REQUIRE_FALSE(machine.is_in(OptionsDone{}));
}
...
TEST_CASE("OptionsSm happy path", "[options_sm]") {
    ...
    REQUIRE(machine.is_in(OptionsResponding{}));
    ...
    REQUIRE(machine.is_in(OptionsDone{}));
    REQUIRE_FALSE(machine.is_in(OptionsResponding{}));
}
...
TEST_CASE("OptionsSm state transitions are deterministic", "[options_sm]") {
    ...
    machine.process_event(MessageReceived{});
    REQUIRE(machine.is_in(OptionsResponding{}));
    REQUIRE_FALSE(machine.is_in(OptionsIdle{}));
