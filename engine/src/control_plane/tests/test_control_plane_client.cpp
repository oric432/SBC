...
TEST_CASE("ControlPlaneClient parses a snapshot envelope") {
    const auto result = ControlPlaneClient::parse_envelope(R"({
        "type": "snapshot",
        "routes_snapshot": {
...
TEST_CASE("ControlPlaneClient rejects malformed JSON") {
    const auto result = ControlPlaneClient::parse_envelope(R"({"type": )");
