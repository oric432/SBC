...
struct DialogSmRunner::Impl {
    using Machine = Sml::sm<DialogSm<DialogActions>, Sml::logger<SmLogger>, Sml::process_queue<std::queue>>;

    Impl(DialogActions& actions, std::string_view call_id)
...
DialogSmRunner::DialogSmRunner(DialogActions& actions, std::string_view call_id)
...
template bool DialogSmRunner::process_event(const Dialog::ReinviteFinished&);
